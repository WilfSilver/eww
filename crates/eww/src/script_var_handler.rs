    ...
    async fn start(&mut self, var: PollScriptVar) {
        ...
        tokio::spawn(async move {
            let result: Result<_> = try {
                evt_send.send(app::DaemonCommand::UpdateVars(None, vec![(var.name.clone(), run_poll_once(&var)?)]))?;
            };
            ...
            crate::try_logging_errors!(format!("Executing listen var-command {}", &var.command) =>  {
                ...
                crate::loop_select_exiting! {
                    ...
                    Ok(Some(line)) = stdout_lines.next_line() => {
                        ...
                        evt_send.send(DaemonCommand::UpdateVars(None, vec![(var.name.to_owned(), new_value)]))?;
                    }
